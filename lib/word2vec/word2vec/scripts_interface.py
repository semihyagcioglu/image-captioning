...
utilities = Utilities()
settings = utilities.load_settings('settings.ini', str('Settings'))
...
def word2vec(train, output, size=100, window=5, sample='1e-3', hs=0, negative=5,
    ...
    process = [applicationPath + 'lib/word2vec/bin/word2vec']
...
def word2phrase(train, output, min_count=5, threshold=100, debug=2, verbose=False):
    process = [applicationPath + 'lib/word2vec/bin/word2phrase']
